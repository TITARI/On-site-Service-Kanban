...
export async function readState(): Promise<AppState> {
  return enqueueStateWrite(() => readStateUnlocked());
}

export async function writeState(state: AppState) {
  return enqueueStateWrite(() => writeStateUnlocked(state));
}
