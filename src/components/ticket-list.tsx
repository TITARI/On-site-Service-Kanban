...
export function TicketList({ tickets, selectedId, onSelect }: { tickets: TicketListItem[]; selectedId?: string; onSelect: (id: string) => void }) {
  const activeTickets = tickets.filter((ticket) => ticket.status !== "已关闭");
  ...
  if (activeTickets.length === 0 && archivedTickets.length === 0) return <section className="empty-state">暂无工单</section>;

  function renderTicketRow(ticket: TicketListItem) {
    ...
    <div className="ticket-groups">
      {Array.from(groups.entries()).map(([issueType, items]) => (
        <section className="issue-group" key={issueType}>
          ...
          {items.map(renderTicketRow)}
        </section>
      ))}
      {archivedTickets.length > 0 && (
        <details className="ticket-archive">
          <summary className="archive-summary">
            <span>归档工单</span>
            <strong>{archivedTickets.length}</strong>
          </summary>
          <section className="archive-list" aria-label="归档工单列表">
            {archivedTickets.map(renderTicketRow)}
          </section>
        </details>
      )}
    </div>
